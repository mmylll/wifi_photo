    ...
    _server.listen(
      (HttpRequest request) async {
        if (request.requestedUri.toString() ==
          ...
          print("这里");
          print(_fileList);
          allowRequest = await senderRequestDialog(context, username, os, _fileList[0]!);
          ...
          processReceiversData({
            ...
            "http://$_address:4040/send-data") {
          eventBus.fire(DataEvent(request.headers['offset']!.first));
          updateData(request.headers['offset']!.first);
        } else
        // todo
        {
